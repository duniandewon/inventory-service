...
- 2026-06-25: Logistics feature documented and started, based on `context/features/logistics.md`.
- 2026-06-26: Implemented on `feature/logistics` branch: new `internal/inventory` domain (transaction-aware mutations only — `ReserveForWorkOrder`, `ProduceFromWorkOrder`, `ConsumeForWorkOrder`, `MarkShipped`, all accepting a `DBTX` so logistics threads its `*sql.Tx` through them); new `internal/logistics` domain (Handler → Service → Repository) owning `process_types`, `work_orders`, `work_order_line_items`, `delivery_notes`, and `delivery_note_items`. All 11 API routes implemented. Added `db/migrations/004_seed_process_types.sql` seeding Cutting/Sewing/Printing. Open decisions resolved: full consumption on receive, single terminal receive step, no cancellation, delivery-note format `DN-YYYY-NNNN`, work-order-level lineage granularity, assign+dispatch as one step. Manually verified end-to-end against dockerized Postgres/Redis: process-type list/create/rename/delete/duplicate-409/in-use-409; work-order create/list/detail/assign-inputs→PROCESSING/receive-outputs→COMPLETED/double-receive-409; delivery-note create (DN-2026-0001)/list/detail/SHIPPED status; lineage tree for shipped item traces back through work order to CONSUMED raw inputs; non-existent 404; unauthenticated 401; status filter. `go build`/`go vet`/`gofmt` clean. Status: Completed.
